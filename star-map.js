    ...
    drawInitialAnimation(progress) {
        ...
        this.constellations.forEach((constellation, constIndex) => {
            ...
            if (constellationProgress > 0.8) {
                ...
                // Texto com brilho mais intenso e fonte maior
                this.ctx.shadowColor = 'rgba(255, 255, 255, 0.8)';
                this.ctx.shadowBlur = 15;
                this.ctx.font = 'italic bold 16px Georgia, serif';
                this.ctx.fillStyle = `rgba(255, 255, 255, ${(0.95 + pulseEffect * 0.05) * nameOpacity})`;
                this.ctx.textAlign = 'center';
                this.ctx.fillText(constellation.name, avgX, maxY + 32);
                
                // Linha decorativa com mais contraste
                const textWidth = this.ctx.measureText(constellation.name).width;
                this.ctx.shadowBlur = 6;
                this.ctx.strokeStyle = `rgba(255, 255, 255, ${(0.6 + pulseEffect * 0.2) * nameOpacity})`;
                this.ctx.lineWidth = 1.2;
                this.ctx.beginPath();
                this.ctx.moveTo(avgX - textWidth / 2 - 8, maxY + 38);
                this.ctx.lineTo(avgX + textWidth / 2 + 8, maxY + 38);
                this.ctx.stroke();
                
                // Pontos nas extremidades mais visíveis
                this.ctx.fillStyle = `rgba(255, 255, 255, ${(0.75 + pulseEffect * 0.25) * nameOpacity})`;
                this.ctx.shadowBlur = 8;
                this.ctx.beginPath();
                this.ctx.arc(avgX - textWidth / 2 - 8, maxY + 38, 2, 0, Math.PI * 2);
                this.ctx.fill();
                this.ctx.beginPath();
                this.ctx.arc(avgX + textWidth / 2 + 8, maxY + 38, 2, 0, Math.PI * 2);
                this.ctx.fill();
