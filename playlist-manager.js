...
function loadTrack(index) {
    ...
    if (coverImg) {
        ...
        // Carregar direto (mais rápido que lazy loading)
...
function preloadAdjacentCovers(currentIndex) {
    ...
    // Índices das próximas 2 músicas
    const nextIndex = (currentIndex + 1) % totalTracks;
    const nextNextIndex = (currentIndex + 2) % totalTracks;
    
    // Pré-carregar capas em background
    [nextIndex, nextNextIndex].forEach(idx => {
        const track = playlist.tracks[idx];
        if (track && track.cover) {
            const img = new Image();
            img.src = track.cover;
            console.log(`🔥 Pré-carregando capa: ${track.title}`);
        }
    });
}
