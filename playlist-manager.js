...
async function initPlaylistManager() {
    ...
    try {
        ...
        console.log(`✅ ${PlaylistManager.customPlaylists.length} playlists carregadas`);
        
        // Aguardar DOM estar pronto
        await waitForPlayer(async () => {
            console.log('✅ DOM pronto para playlists');
        ...
        console.error('❌ Erro ao inicializar playlists:', error);
        
        // Mostrar mensagem de erro
        const loadingState = document.getElementById('playlistLoadingState');
        if (loadingState) {
...
function waitForPlayer(callback) {
    return new Promise((resolve) => {
        ...
        const checkInterval = setInterval(() => {
            ...
            const dynamicContainer = document.getElementById('playlistDynamicContainer');
            
            if (playerSection && dynamicContainer) {
                clearInterval(checkInterval);
                ...
                clearInterval(checkInterval);
                console.error('❌ Timeout aguardando elementos do player');
                resolve();
            }
        }, 100);
    });
...
function createPlaylistNavigation() {
    ...
    if (!playerSection || !dynamicContainer || document.getElementById('playlistCarousel')) return;
    ...
    playlistContainer.innerHTML = `
        ...
        <div class="playlist-carousel-wrapper">
            <div class="playlist-carousel" id="playlistCarousel">
                <!-- Playlists serão criadas dinamicamente -->
            </div>
        </div>
    `;
    
    dynamicContainer.appendChild(playlistContainer);
    
    console.log('✅ Container de playlists criado');
}
...
function switchToPlaylist(index) {
    ...
    // Carregar tracks da playlist
    loadCustomPlaylistTracks(index);
}
...
async function renderCustomPlaylists() {
    ...
    console.log(`🎨 Renderizando ${PlaylistManager.customPlaylists.length} playlists...`);
    
    // Limpar containers
    carousel.innerHTML = '';
    indicatorsContainer.innerHTML = '';
    
    // Verificar se há playlists
    if (PlaylistManager.customPlaylists.length === 0) {
        // Mostrar mensagem de playlist vazia
        carousel.innerHTML = `
            <div class="playlist-slide active">
                <div class="empty-playlist-message">
                    <i class="fas fa-music"></i>
                    <h3>Nenhuma playlist ainda</h3>
                    <p>Crie sua primeira playlist no painel de admin!</p>
                </div>
            </div>
        `;
        
        // Esconder loading e mostrar container
        if (loadingState) loadingState.style.display = 'none';
        if (dynamicContainer) dynamicContainer.style.display = 'block';
        
        console.log('ℹ️ Nenhuma playlist encontrada');
        return;
    }
    ...
    // Carregar primeira playlist
    if (PlaylistManager.customPlaylists.length > 0) {
        loadCustomPlaylistTracks(0);
    }
    ...
    console.log('✅ Playlists renderizadas com sucesso!');
}
