...
async function loadAlbumsFromFirebase() {
    ...
    try {
        ...
        // Fazer fetch das fotos em paralelo (evita N+1 queries)
        ...
        snapshot.docs.forEach((doc, index) => {
            ...
            firebaseAlbums.push({
                ...
                coverThumb: albumData.coverThumb, // Added
                coverLarge: albumData.coverLarge, // Added
        ...
        if (firebaseAlbums.length === 0) {
            console.log('⚠️ Nenhum álbum encontrado no Firebase.');
        }
...
async function forceLoadAlbums() {
    ...
    if (typeof firebase === 'undefined' || !firebase.apps.length) {
        ...
        if (forceLoadRetries > 30) {
            console.error('❌ Firebase não carregou após 15 segundos. Desistindo de forceLoadAlbums.');
            return;
        }
        console.warn(`⚠️ Firebase ainda não está pronto, aguardando... (tentativa ${forceLoadRetries})`);
