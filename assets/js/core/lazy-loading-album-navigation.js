...
const IntelligentAlbumLoader = {
    ...
    config: {
        ...
        albumTransitionThreshold: 3, // Distância do fim para pré-carregar próximo álbum
        maxCacheSize: 20,          // Máximo de imagens em cache
    ...
    async preloadAdjacentPhotos() {
        ...
        // 3. INTELIGÊNCIA: Se está perto do FIM, pré-carregar próximo álbum
        ...
        if (distanceToEnd <= this.config.albumTransitionThreshold) {
            ...
            if (nextAlbum && nextAlbum.photos && nextAlbum.photos.length > 0) {
                console.log(`🔮 Perto do fim - pré-carregando próximo álbum`);
        ...
        // 4. INTELIGÊNCIA: Se está perto do INÍCIO, pré-carregar álbum anterior
        if (currentIndex <= this.config.albumTransitionThreshold) {
            const prevAlbum = this.getPreviousAlbum();
            
            if (prevAlbum && prevAlbum.photos && prevAlbum.photos.length > 0) {
                console.log(`🔮 Perto do início - pré-carregando álbum anterior`);
