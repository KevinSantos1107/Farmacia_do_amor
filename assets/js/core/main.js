...
function initTimeCounter() {
    ...
    window.timeCounterInterval = setInterval(updateTimeCounter, 1000);
}
...
async function openAlbum(albumId) {
    ...
    if (!currentAlbum.photos) {
        try {
            console.log(`Carregando fotos do álbum ${currentAlbum.title}...`);
            const photoSnap = await db.collection('album_photos')
            ...
            photoSnap.docs.forEach(doc => {
                allPhotos.push(...doc.data().photos);
            });
            currentAlbum.photos = allPhotos;
        } catch (error) {
            console.error('Erro ao carregar fotos:', error);
            window.showCustomAlert('Erro', 'Não foi possível carregar as fotos do álbum.');
            return;
        }
    }
...
function initHamburgerMenu() {
    ...
    modals.forEach(modal => {
        if (modal) {
            window.modalObserver.observe(modal, { attributes: true, attributeFilter: ['style'] });
        }
    });
