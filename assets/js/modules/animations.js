    ...
    const settings = {
        ...
        winter: {
            name: 'Nevasca Neon',
            snowflakes: 70, // Meio (flakes nítidos)
            smallSnow: 120, // Fundo (névoa lenta)
            bokehSnow: 20, // Frente (blur grande)
            sparkles: 40,
            snowColors: ['#ffffff', '#cce6ff', '#e0f2fe', '#00f2fe'],
            sparkleColors: ['#00f2fe', '#4facfe', '#ffffff'],
            skyGradient: [
                '#030b16',
                '#041020',
                '#0a1d3a'
...
function resizeCanvas() {
    ...
    function createMainSnowflake() {
        ...
        particles.push({
            ...
            glow: Math.random() * config.glowIntensity + 0.3,
            age: 0
        });
    }
    ...
    function createBokehSnowflake() {
        ...
        particles.push({
            ...
            speedY: Math.random() * config.snowSpeed * 2.5 + 1.5,
            speedX: (Math.random() - 0.5) * config.windStrength * 2,
            color: config.snowColors[Math.floor(Math.random() * config.snowColors.length)],
            opacity: Math.random() * 0.15 + 0.05,
            swing: Math.random() * Math.PI * 2,
            swingSpeed: Math.random() * 0.005 + 0.002,
            swingAmplitude: Math.random() * 3.0 + 1.0,
            age: 0
        });
    }
    ...
    function drawBokehSnowflakes() {
        const time = Date.now();
        
        particles.forEach(particle => {
            if (particle.type !== 'bokehSnow') return;
    ...
    function drawWinterFog() {
        ...
        // Fog suave debaixo para cima, sem linhas duras
        const gradient = ctx.createLinearGradient(0, canvas.height * 0.3, 0, canvas.height);
        gradient.addColorStop(0, 'rgba(3, 11, 22, 0)');
        gradient.addColorStop(0.5, 'rgba(4, 16, 32, 0.4)');
    ...
    function animate() {
        ...
        if (currentAnimation === 'winter') {
            ...
            // Fundo base escuridão do ártico
            ctx.fillStyle = '#020813';
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            
            // Luz neon ciano (movendo-se suavemente pelo topo)
            const cx1 = canvas.width * (0.5 + Math.sin(time * 0.5) * 0.3);
            const cy1 = canvas.height * (0.2 + Math.cos(time * 0.3) * 0.1);
            const grad1 = ctx.createRadialGradient(cx1, cy1, 0, cx1, cy1, canvas.width * 0.8);
            grad1.addColorStop(0, 'rgba(0, 242, 254, 0.15)'); 
            grad1.addColorStop(1, 'rgba(0, 242, 254, 0)');
            ctx.fillStyle = grad1;
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            
            // Sombra/profundidade anil profundo
            const cx2 = canvas.width * (0.5 + Math.cos(time * 0.4) * 0.3);
            const cy2 = canvas.height * (0.8 + Math.sin(time * 0.6) * 0.2);
            const grad2 = ctx.createRadialGradient(cx2, cy2, 0, cx2, cy2, canvas.width * 0.9);
            grad2.addColorStop(0, 'rgba(10, 29, 58, 0.9)'); 
            grad2.addColorStop(1, 'rgba(10, 29, 58, 0)');
            ctx.fillStyle = grad2;
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            
            // Nuvem/aurora azul claro inferior
            const grad3 = ctx.createRadialGradient(canvas.width * 0.5, canvas.height, 0, canvas.width * 0.5, canvas.height, canvas.width);
            grad3.addColorStop(0, 'rgba(79, 172, 254, 0.12)'); 
            grad3.addColorStop(1, 'rgba(79, 172, 254, 0)');
            ctx.fillStyle = grad3;
            ctx.fillRect(0, 0, canvas.width, canvas.height);
