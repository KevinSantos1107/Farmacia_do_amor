...
const AudioManager = {
    ...
    play(audioElement, playerId) {
        if (this.currentAudio && this.currentAudio !== audioElement) {
            if (this.currentPlayerId) {
                this.cleanupPlayer(this.currentPlayerId);
            } else {
    ...
    cleanupPlayer(playerId) {
        ...
        try {
            ...
            if (player) {
                ...
                const timeEl = player.querySelector('.time-display span:first-child');
    ...
    cleanupAll() {
        ...
        this.currentAudio = null;
        this.currentPlayerId = null;
        console.log('🧹 Cleanup total: Todos os players resetados');
    }
};
...
async function loadPlaylistsFromFirebase() {
    try {
        ...
        const playlistIds = sortedDocs.map(doc => doc.id);
        
        // Fazer fetch das tracks em paralelo (evita N+1 queries)
        const tracksPromises = playlistIds.map(id => 
            db.collection('playlist_tracks')
                .where('playlistId', '==', id)
                .get()
        );
        const tracksSnapshots = await Promise.all(tracksPromises);
        
        sortedDocs.forEach((doc, index) => {
            const playlistData = doc.data();
            console.log(`📂 Carregando músicas da playlist: ${playlistData.name}`);
            
            const sortedTrackDocs = Array.from(tracksSnapshots[index].docs).sort((a, b) => {
                return (a.data().pageNumber || 0) - (b.data().pageNumber || 0);
            });
            
            const allTracks = [];
            sortedTrackDocs.forEach(trackDoc => {
                const tracksArray = trackDoc.data().tracks;
                if (Array.isArray(tracksArray)) {
                    allTracks.push(...tracksArray);
                } else if (trackDoc.data().title) {
                    allTracks.push(trackDoc.data());
                }
            });
            ...
            PlaylistManager.customPlaylists.push({
                ...
                cover: playlistData.cover || 'assets/images/capas-albuns/nossa-trilha.jpg',
                tracks: allTracks
            });
        });
...
function createCustomPlayer(container, playlist, playlistIndex) {
    ...
    container.innerHTML = `
        <div class="music-player" id="${playerId}">
            ...
            <audio id="${audioId}" preload="metadata" playsinline webkit-playsinline></audio>
        </div>
    `;
...
function initCustomPlayerControls(playerId, audioId, playlist) {
    ...
    const isThisPlaylistPlaying = () => AudioManager.currentPlayerId === playerId;
    ...
    playPauseBtn.addEventListener('click', () => {
        const playingHere = isThisPlaylistPlaying();
    ...
    prevBtn.addEventListener('click', () => {
        if (isLoading) return;
        const playingHere = isThisPlaylistPlaying();
        const isCurrentlyPlaying = playingHere && !audio.paused;
        
        if (playingHere && audio.currentTime > 3) {
            audio.currentTime = 0;
        } else {
            ...
            loadTrack(currentTrackIndex, isCurrentlyPlaying);
        }
    });
    
    nextBtn.addEventListener('click', () => {
        if (isLoading) return;
        const isCurrentlyPlaying = isThisPlaylistPlaying() && !audio.paused;
        goToNextTrack(isCurrentlyPlaying);
    });
    ...
    function updatePlaybackUI(playing) {
        ...
        document.getElementById(playerId).classList.toggle('playing', Boolean(playing));
    }
    ...
    function playLoadedTrack() {
        ...
        if (playPromise && typeof playPromise.then === 'function') {
            playPromise.then(() => {
                ...
                // A atualização da Media Session já ocorre em loadTrack() e nos eventos de play
            }).catch(err => {
                isPlaying = false;
                updatePlaybackUI(false);
                console.warn('Erro ao tocar:', err.message);
            });
        } else {
            isPlaying = true;
            updatePlaybackUI(true);
    ...
    function loadTrack(index, shouldPlay = isPlaying) {
        if (isLoading) return;
        ...
        if (window.MediaControlsManager && (shouldPlay || isThisPlaylistPlaying())) {
        ...
        if (coverImg) {
            const newCover = track.cover || playlist.cover || 'assets/images/capas-albuns/default-music.jpg';
...
class MediaControlsManager {
    ...
    _pushMetadata(player) {
        ...
        // ✅ Evitar atualizações e logs redundantes da mesma música
        const currentMeta = navigator.mediaSession.metadata;
        if (currentMeta && currentMeta.title === title && currentMeta.artist === artist) {
            const currentArtwork = currentMeta.artwork && currentMeta.artwork.length > 0 ? currentMeta.artwork[0].src : '';
            if (currentArtwork === artworkSrc) {
                return; // Já está atualizado, não precisa recriar a MediaMetadata nem logar
            }
        }
