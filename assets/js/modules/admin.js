...
function initAlbumForms() {
    ...
    addAlbumForm.addEventListener('submit', async (e) => {
        ...
        if (coverFile.size > 32 * 1024 * 1024) {
            showAdminToast('❌ Imagem muito grande! Limite de 32MB por imagem.', "error");
            return;
        }
            ...
            for (let i = 0; i < compressedFiles.length; i += CONCURRENT_UPLOADS) {
                ...
                const batchPromises = batch.map(async (file, idx) => {
                    ...
                    try {
                        ...
                        const exif  = exifDataByIndex[globalIndex] || {};
                        return {
                            src:         urls.medium,
                            srcThumb:    urls.thumb,
                            srcLarge:    urls.large,
...
window.deletePlaylist = async function(playlistId, playlistName) {
    const isConfirmed = await window.showCustomConfirm("Ateno", "Tem certeza que deseja deletar a playlist?\n\nIsso remover TODAS as msicas desta playlist.");
    if (!isConfirmed) {
        return;
    }
...
async function addMusicToPlaylist(e) {
    ...
    // Confirmao para muitos arquivos
    if (audioFiles.length > 20) {
        const confirm = await window.showCustomConfirm(
            'Aviso',
            `?? Voc selecionou ${audioFiles.length} msicas!\n\nIsso pode demorar bastante tempo.\nDeseja continuar?`
        );
        if (!confirm) return;
    }
...
async function deleteAlbumFromGrid(id, title) {
    const isConfirmed = await window.showCustomConfirm('Excluir lbum', `Tem certeza que deseja excluir o lbum "${title}" e TODAS as suas fotos? Esta ao no pode ser desfeita.`);
    if (isConfirmed) {
        try {
