...
function showAdminToast(message, type = 'info', duration = 4000) {
    ...
    // Clean emoji prefixes from message
    let cleanMsg = message.replace(/^[\u2705\u274c\u26a0\ufe0f\ud83d\udcc2\ud83d\udcbe\ud83c\udfb5\u2728\ud83d\udd12\ud83c\udf1f\ud83d\ude80\ud83d\udcf7\ud83e\uddf9\ud83d\udd25\ud83c\udfa8\ud83d\udc4d\ud83d\udd04\ud83d\udca1\ud83c\udf89\ud83d\udce6⚠️❌✅📂🗑️🎵✨🔐🎶📷🧹🔥🎨👍🔄💡🎉📦]+\s*/, '');
    
    const toast = document.createElement('div');
    toast.className = 'admin-toast ' + type;
    toast.innerHTML = 
        '<div class="admin-toast-icon"><i class="fas ' + (icons[type] || icons.info) + '"></i></div>' +
        '<div class="admin-toast-message">' + cleanMsg + '</div>' +
    ...
    toast.addEventListener('click', function(e) {
        if (!e.target.closest('.admin-toast-close')) {
            toast.classList.add('toast-exit');
            setTimeout(function() { toast.remove(); }, 300);
        }
    });
...
// === FILE PREVIEW FOR ADD PHOTOS ===
function updateFilePreview(files) {
    const info = document.getElementById('selectedFilesInfo');
    const grid = document.getElementById('photoPreviewGrid');
    if (info) info.textContent = files.length > 0 ? files.length + ' arquivo(s) selecionado(s)' : 'Nenhum arquivo selecionado';
    if (grid) {
        grid.innerHTML = '';
        Array.from(files).slice(0, 20).forEach(function(file) {
        ...
        if (files.length > 20) {
            ...
            more.style.cssText = 'display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.5);font-size:0.85em;';
...
// === INLINE TOOLBAR UPDATE ===
function updateInlineToolbar() {
    const checkboxes = document.querySelectorAll('#editPhotosGrid input[type="checkbox"]:checked');
    const count = checkboxes.length;
    const countSpan = document.getElementById('editDeleteCount');
    const actionBar = document.getElementById('photoSelectionActionBar');
...
// Senha de admin ofuscada (temporário, usar Firebase Auth no futuro)
const ADMIN_PASSWORD_HASH = 'aWFyYTIwMjU=';
...
function openAdminLogin() {
    ...
    if (!loginModal || !loginForm || !passwordInput) {
        ...
        window.showCustomAlert('Erro', 'Modal de login indisponível.').then(() => {});
        return;
    }
    ...
    const onSubmit = async (evt) => {
        ...
        if (btoa(pwd) === ADMIN_PASSWORD_HASH) {
            cleanUp();
            ...
            if (errorDiv) {
                ...
                showAdminToast('❌ Senha incorreta!', "error");
            }
            passwordInput.value = '';
            passwordInput.focus();
        }
...
function waitForServices() {
    return new Promise((resolve) => {
        const maxWait = 15000; // 15 segundos de timeout máximo
        ...
        const checkInterval = setInterval(() => {
            if (typeof firebase !== 'undefined' && 
                ...
                clearInterval(checkInterval);
                console.warn('⚠️ waitForServices: timeout — continuando mesmo sem todos os serviços.');
                resolve();
            }
        }, 100);
    });
...
function initAlbumForms() {
    ...
    addAlbumForm.addEventListener('submit', async (e) => {
        ...
        if (!coverFile) {
            showAdminToast('❌ Selecione uma imagem de capa!', "warning");
            return;
        }
        ...
        try {
            ...
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Preparando capa...';
            btn.disabled = true;
            
            let processedCover = coverFile;
            try {
                processedCover = await compressImageIfNeeded(coverFile, 10);
            } catch (err) {
                console.warn('⚠️ Erro ao comprimir capa, usando original:', err);
            }
            
            if (processedCover.size > 32 * 1024 * 1024) {
                showAdminToast('❌ Foto da capa ainda muito grande!', "error");
            ...
            const coverUrls = await uploadImageToCloudinary(processedCover, 1600, true);
            ...
            showAdminToast(`✅ Álbum "${title}" criado com versões responsivas!`, "success");
            addAlbumForm.reset();
            btn.innerHTML = originalText;
            btn.disabled = false;
            ...
            if (typeof renderAdminAlbums === 'function') renderAdminAlbums();
            document.getElementById('newAlbumModal').style.display = 'none';
            
        } catch (error) {
            console.error('❌ Erro ao criar álbum:', error);
            showAdminToast('❌ Erro ao criar álbum: ' + error.message, "error");
            const btn = addAlbumForm.querySelector('button');
    ...
    addPhotoForm.addEventListener('submit', async (e) => {
        ...
        if (!albumId) {
            showAdminToast('❌ Selecione um álbum primeiro!', "warning");
            return;
        }
        
        if (photoFiles.length === 0) {
            showAdminToast('❌ Selecione pelo menos uma foto!', "warning");
            return;
        }
        ...
        try {
            ...
            for (let i = 0; i < photoFiles.length; i++) {
                try {
                    ...
                    compressedFiles.push({ file: compressed, originalIndex: i });
            ...
            // ETAPA 1.5: EXTRAIR METADADOS EXIF DOS ARQUIVOS ORIGINAIS
            btn.innerHTML = '<i class="fas fa-map-marker-alt fa-spin"></i> Lendo metadados das fotos...';
            const exifDataByIndex = {};
            if (typeof exifr !== 'undefined') {
                const exifPromises = Array.from(photoFiles).map(async (file, i) => {
                    try {
                        const exif = await exifr.parse(file, {
                            pick: ['DateTimeOriginal', 'GPSLatitude', 'GPSLongitude',
                                   'GPSLatitudeRef', 'GPSLongitudeRef', 'CreateDate']
                        });
                        if (!exif) return;
                        let dateTaken = '';
                        const rawDate = exif.DateTimeOriginal || exif.CreateDate;
                        if (rawDate) {
                            dateTaken = rawDate instanceof Date
                                ? rawDate.toISOString()
                                : new Date(String(rawDate)).toISOString();
                        }
                        let location = '';
                        if (exif.GPSLatitude && exif.GPSLongitude) {
                            const lat = _dmsToDecimal(exif.GPSLatitude, exif.GPSLatitudeRef || 'N');
                            const lng = _dmsToDecimal(exif.GPSLongitude, exif.GPSLongitudeRef || 'E');
                            if (lat !== null && lng !== null) {
                                location = await _reverseGeocode(lat, lng);
                            }
                        }
                        exifDataByIndex[i] = { dateTaken, location };
                        if (dateTaken || location) {
                            console.log(`📍 EXIF foto ${i + 1}: data="${dateTaken}", local="${location}"`);
                        }
                    } catch (e) { /* sem EXIF — ok */ }
                });
                await Promise.allSettled(exifPromises);
            } else {
                console.warn('⚠️ exifr não carregado — metadados EXIF não serão extraídos.');
            }
            ...
            for (let i = 0; i < compressedFiles.length; i += CONCURRENT_UPLOADS) {
                ...
                const batchPromises = batch.map(async (item, idx) => {
                    ...
                    try {
                        ...
                        const exif  = exifDataByIndex[originalIndex] || {};
                        return {
                            src:         urls.medium,
                            srcThumb:    urls.thumb,
                            srcLarge:    urls.large,
                            srcWebP:     urls.webp,
                            description: '',
                            timestamp:   Date.now() + originalIndex,
                            dateTaken:   exif.dateTaken || '',
                            location:    exif.location  || '',
                        };
                    } catch (uploadError) {
                        console.error(`❌ Erro no upload da foto ${originalIndex + 1}:`, uploadError);
            ...
            if (photoUrls.length === 0) {
                showAdminToast('❌ Nenhuma foto foi enviada com sucesso!', "success", "success");
                btn.innerHTML = originalText;
                btn.disabled = false;
                return;
            }
            ...
            await db.collection('albums').doc(albumId).update({
                photoCount: firebase.firestore.FieldValue.increment(photoUrls.length)
            });
            ...
            showAdminToast('❌ Erro ao adicionar fotos: ' + error.message, "error");
            btn.innerHTML = originalText;
            btn.disabled = false;
        }
    });
...
/* Timeline form logic moved to js/timeline.js */
...
async function loadExistingAlbums() {
    ...
    if (!container) return; // Prevent error if element is removed from UI
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            const safeTitle = album.title ? album.title.replace(/</g, '&lt;').replace(/>/g, '&gt;') : '';
            const safeDate = album.date ? album.date.replace(/</g, '&lt;').replace(/>/g, '&gt;') : '';
            
            item.innerHTML = `
                <div class="existing-item-info">
                    <div class="existing-item-title">${safeTitle}</div>
                    <div class="existing-item-meta">${safeDate} • ${album.photoCount || 0} fotos</div>
                </div>
...
/* Timeline admin event loading moved to js/timeline.js */

window.deleteAlbum = async function(albumId) {
    const isConfirmed = await window.showCustomConfirm('Confirmação', 'Tem certeza de que deseja excluir este álbum? Esta ação remove o álbum do painel.');
    if (!isConfirmed) {
        return;
    }
    ...
    try {
        ...
        // Deletar o álbum SOMENTE DEPOIS de deletar as fotos
        await db.collection('albums').doc(albumId).delete();
        
        showAdminToast('Álbum excluído com sucesso.', "success");
        loadExistingContent();
        updateAlbumSelect();
        await loadAlbumsFromFirebase();
        if (typeof renderAdminAlbums === 'function') renderAdminAlbums();
        ...
        showAdminToast('❌ Erro ao excluir: ' + error.message, "error");
    }
};
...
function initializeAlbumEditSection() {
    ...
    // Floating action bar (new UI)
    const cancelSelBtn = document.getElementById('editCancelSelBtn');
    if (cancelSelBtn) cancelSelBtn.addEventListener('click', cancelSelection);

    const deleteFabBtn = document.getElementById('editDeleteBtn');
    if (deleteFabBtn) deleteFabBtn.addEventListener('click', deleteSelectedPhotos);
    ...
    const floatingCancelBtn = document.getElementById('editCancelSelBtn');
    if (floatingCancelBtn) floatingCancelBtn.addEventListener('click', cancelSelection);

    const floatingDeleteBtn = document.getElementById('editDeleteBtn');
    if (floatingDeleteBtn) floatingDeleteBtn.addEventListener('click', deleteSelectedPhotos);
...
function recreateToolbarListeners() {
    // Re-bind the floating action bar buttons after album reload
    const cancelBtn = document.getElementById('editCancelSelBtn');
    if (cancelBtn) {
        const newBtn = cancelBtn.cloneNode(true);
        cancelBtn.parentNode.replaceChild(newBtn, cancelBtn);
        newBtn.addEventListener('click', cancelSelection);
    }
    ...
    if (deleteBtn) {
        ...
        newBtn.addEventListener('click', deleteSelectedPhotos);
    }
}

async function loadAlbumForEdit(e, forceId = null) {
    const select = document.getElementById('editAlbumSelect');
    const albumId = forceId || (select ? select.value : null);
    ...
    try {
        ...
        // Hide the floating action bar if open
        const actionBar = document.getElementById('photoSelectionActionBar');
        ...
        showAdminToast('❌ Erro ao carregar álbum: ' + error.message, "error");
...
function previewNewCover(e) {
    ...
    if (file.size > 32 * 1024 * 1024) {
        showAdminToast('❌ Imagem muito grande! Máximo 32MB.', "error");
        return;
    }
    ...
    const coverImg = document.getElementById('currentCoverPreview');
    if (coverImg) {
        coverImg.onload = () => URL.revokeObjectURL(coverImg.src);
...
async function saveAlbumInfo() {
    ...
    try {
        ...
        if (!newTitle || !newDate) {
            showAdminToast('⚠️ Título e Data são obrigatórios!');
        ...
        if (newCoverFile) {
            saveBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i><span>Preparando capa...</span>';
            let processedCover = newCoverFile;
            try {
                processedCover = await compressImageIfNeeded(newCoverFile, 10);
            } catch (err) {
                console.warn('⚠️ Erro ao comprimir capa editada:', err);
            }
        ...
        showAdminToast('✅ Informações do álbum atualizadas com sucesso!', "success", "success");
        ...
        showAdminToast('❌ Erro ao salvar: ' + error.message, "error");
...
function renderPhotosForEdit(photos, albumTitle) {
    ...
    photos.forEach((photo, index) => {
        ...
        photoCard.addEventListener('dragend', (e) => {
            handleDragEnd.call(photoCard, e);
            setTimeout(() => { wasDragged = false; }, 50);
        });
        photoCard.addEventListener('touchstart', handleTouchStart, { passive: true });
        photoCard.addEventListener('touchmove', handleTouchMove, { passive: false });
        photoCard.addEventListener('touchend', handleTouchEnd, { passive: true });
    });
    ...
    if (badge) badge.textContent = photos.length > 0 ? `${photos.length} fotos` : '';
}
...
let isSelectionStatePushed = false;

function updateSelectionUI() {
    const checkboxes = document.querySelectorAll('#editPhotosGrid input[type="checkbox"]');
    const selectedCount = Array.from(checkboxes).filter(cb => cb.checked).length;
    
    // Use the new floating action bar
    const actionBar = document.getElementById('photoSelectionActionBar');
    const deleteCountEl = document.getElementById('editDeleteCount');
    
    if (selectedCount > 0) {
        if (!isSelectionStatePushed) {
            if (typeof HistoryManager !== 'undefined') {
                HistoryManager.push('edit-mode-selection');
            }
            isSelectionStatePushed = true;
        }
        ...
        if (isSelectionStatePushed && typeof HistoryManager !== 'undefined') {
            HistoryManager.remove('edit-mode-selection');
        }
        isSelectionStatePushed = false;
        if (actionBar) actionBar.style.display = 'none';
...
function setupBackButtonHandler() {
    // Push state when any modal opens so the back button works
        ...
        mutations.forEach((mutation) => {
            if (mutation.attributeName === 'style') {
                ...
                if (target.style.display !== 'none') {
                    if (typeof HistoryManager !== 'undefined') {
                        // Prevent duplicates
                        HistoryManager.remove(target.id);
                        HistoryManager.push(target.id);
                    }
                } else {
                    if (typeof HistoryManager !== 'undefined') {
                        HistoryManager.remove(target.id);
    ...
    const modalsToObserve = ['editAlbumModal', 'newAlbumModal', 'addPhotosModal'];
    modalsToObserve.forEach(id => {
...
function enterReorganizeMode() {
    // Drag and drop is always active now — this function is kept for compatibility
    isReorganizing = true;
    cancelSelection();
    console.log('📝 Modo reorganizar ativado (drag livre)');
}

function exitReorganizeMode(save = false) {
    isReorganizing = false;
    if (save) saveNewPhotoOrder();
...
function handleDrop(e) {
    ...
    if (draggedIndex !== targetIndex) {
        ...
        if (window.photoOrderSaveTimeout) clearTimeout(window.photoOrderSaveTimeout);
        window.photoOrderSaveTimeout = setTimeout(() => {
            saveNewPhotoOrder(true);
        }, 1000);
    }
...
function swapPhotos(fromIndex, toIndex) {
    ...
    renderPhotosForEdit(photos, window.currentEditAlbum.data.title);
...
async function saveNewPhotoOrder(silent = false) {
    if (!window.currentEditAlbum) return;
    ...
    try {
        if (!silent) {
            const btn = document.getElementById('savePhotoOrderBtn');
        ...
        for (let pageIndex = 0; pageIndex < newPages.length; pageIndex++) {
            await db.collection('album_photos').add({
                ...
                photos: newPages[pageIndex].map((p, idx) => ({
                    ...
                    timestamp:   p.timestamp   || (Date.now() + (pageIndex * PHOTOS_PER_PAGE) + idx),
        ...
        if (!silent) {
            showAdminToast('✅ Nova ordem das fotos salva com sucesso!', "success", "success");
            
            const select = document.getElementById('editAlbumSelect');
            if (select) select.value = albumId;
            
            const loadBtn = document.getElementById('loadEditAlbumBtn');
            if (loadBtn) loadBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Recarregando...';
            
            await new Promise(resolve => setTimeout(resolve, 500));
            
            await loadAlbumForEdit(null, albumId);
            await updateEditAlbumSelect();
            if (typeof renderAdminAlbums === 'function') renderAdminAlbums();
            
            console.log('✅ Álbum recarregado com nova ordem!');
        } else {
            console.log('💾 Ordem salva silenciosamente no background.');
        }
...
function updateSelectionCount() {
    // Delegated to updateSelectionUI
    updateSelectionUI();
}
...
async function deleteSelectedPhotos() {
    ...
    if (checkboxes.length === 0) {
        showAdminToast('⚠️ Selecione pelo menos uma foto para deletar!', "warning");
        return;
    }
    ...
    const isConfirmed = await window.showCustomConfirm('Confirmação', confirmMsg + '\n\nTem certeza? Esta ação remove as imagens selecionadas do painel.');
    if (!isConfirmed) {
        return;
    }
    
    const currentAlbumId = window.currentEditAlbum.id;
    // Use the new floating action bar delete button
    const btn = document.getElementById('editDeleteBtn');
    const actionBar = document.getElementById('photoSelectionActionBar');
    
    try {
        if (btn) { btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Deletando...'; btn.disabled = true; }
        ...
        showAdminToast(`${selectedIndices.length} foto(s) removida(s) com sucesso.`, "success");
        
        if (btn) { btn.innerHTML = '<i class="fas fa-trash"></i> Deletar'; btn.disabled = false; }
        if (actionBar) actionBar.style.display = 'none';
        ...
        await loadAlbumForEdit(null, currentAlbumId);
        ...
        showAdminToast('❌ Erro ao deletar fotos: ' + error.message, "error");
...
function injectEditStyles() {
    ...
    style.textContent = `
        .edit-photos-grid.google-photos-style {
            display: grid;
        ...
        /* Hover effect like Google Photos (dark overlay when hovering) */
        .gallery-photo::after {
            content: '';
            position: absolute;
            inset: 0;
            background: linear-gradient(to bottom, rgba(0,0,0,0.4) 0%, rgba(0,0,0,0) 30%, rgba(0,0,0,0) 100%);
            opacity: 0;
        ...
        /* Checkmark on TOP LEFT like Google Photos */
        .photo-checkmark {
            position: absolute;
            top: 6px;
            left: 6px;
        ...
        /* Show checkmark outline on hover */
        .gallery-photo:hover .photo-checkmark, .gallery-photo.selection-mode .photo-checkmark {
            opacity: 1;
        }
        ...
        /* Zoom out image slightly when selected */
        ...
        /* Floating Action Bar - inside modal, sticky at bottom of grid */
        .photo-selection-action-bar {
            position: sticky;
            bottom: 0;
            left: 0;
            right: 0;
        ...
        .close-selection-btn:hover { color: white; }
        ...
        .action-btn.delete-btn { background: rgba(244, 67, 54, 0.2); color: #ff5252; }
        .action-btn.delete-btn:hover { background: rgba(244, 67, 54, 0.3); }
...
function addPlaylistTabToAdmin() {
    ...
    const openNewPlaylistBtn = document.getElementById('openNewPlaylistModalBtn');
    const closeNewPlaylistBtn = document.getElementById('closeNewPlaylistModalBtn');
    
    if (openNewPlaylistBtn && newPlaylistModal) {
        openNewPlaylistBtn.addEventListener('click', () => { newPlaylistModal.style.display = 'flex'; });
    }
    if (closeNewPlaylistBtn && newPlaylistModal) {
        closeNewPlaylistBtn.addEventListener('click', () => { newPlaylistModal.style.display = 'none'; });
    }

    const addMusicModal = document.getElementById('addMusicModal');
    const openAddMusicBtn = document.getElementById('openAddMusicModalBtn');
    const closeAddMusicBtn = document.getElementById('closeAddMusicModalBtn');
    
    if (openAddMusicBtn && addMusicModal) {
        openAddMusicBtn.addEventListener('click', () => { addMusicModal.style.display = 'flex'; });
    }
    if (closeAddMusicBtn && addMusicModal) {
        closeAddMusicBtn.addEventListener('click', () => { addMusicModal.style.display = 'none'; });
    }

    // Auto-close when clicking outside
    window.addEventListener('click', (e) => {
        if(newPlaylistModal && e.target === newPlaylistModal) newPlaylistModal.style.display = 'none';
        if(addMusicModal && e.target === addMusicModal) addMusicModal.style.display = 'none';
    });

    console.log('✅ Aba de playlists ativada e modais configurados');
}
...
async function createNewPlaylist(e) {
    ...
    if (!name) {
        showAdminToast('⚠️ Digite o nome da playlist!', "warning");
        return;
    }
...
showAdminToast(`✅ Playlist "${name}" criada com sucesso!`, "success", "success");
...
if(document.getElementById('newPlaylistModal')) {
...
async function loadExistingPlaylists() {
    ...
    try {
        ...
        if (snapshot.empty) {
            container.innerHTML = `
                <div class="retro-admin-loading" style="grid-column: 1 / -1;">
        ...
        for (const doc of snapshot.docs) {
            ...
            card.innerHTML = `
                <div class="admin-playlist-card-icon">
                    <i class="fas ${playlist.icon}"></i>
                </div>
                
                <div class="admin-playlist-card-overlay">
                    <h4 class="admin-playlist-card-title">${playlist.name}</h4>
                    <div class="admin-playlist-card-meta">
                        <span><i class="fas fa-music"></i> ${playlist.trackCount || 0} música${playlist.trackCount !== 1 ? 's' : ''}</span>
                    </div>
                    
                    <div class="admin-playlist-card-actions">
                        <button class="admin-playlist-action-btn edit" onclick="editPlaylist('${doc.id}')" title="Editar">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="admin-playlist-action-btn delete" onclick="deletePlaylist('${doc.id}', '${playlist.name}')" title="Deletar">
        ...
        container.innerHTML = `
            <div class="retro-admin-loading" style="grid-column: 1 / -1; color: #ff5050;">
...
window.deletePlaylist = async function(playlistId, playlistName) {
    const isConfirmed = await window.showCustomConfirm("Atenção", "Tem certeza que deseja deletar a playlist?\n\nIsso removerá TODAS as músicas desta playlist.");
    if (!isConfirmed) {
        return;
    }
    ...
    try {
        ...
        showAdminToast(`✅ Playlist "${playlistName}" deletada com sucesso!`, "success", "success");
...
window.editPlaylist = function(playlistId) {
    if (typeof playlistEditManager !== 'undefined') {
        ...
        showAdminToast('⚠️ Sistema de edição não está pronto. Recarregue a página.', "warning");
    }
};
...
function initAdminSystems() {
    ...
    if (typeof renderAdminAlbums === 'function') renderAdminAlbums();
    if (typeof initAlbumModals === 'function') initAlbumModals();
}
...
async function addMusicToPlaylist(e) {
    ...
    if (!playlistId) {
        showAdminToast('⚠️ Selecione uma playlist!', "warning");
        return;
    }
    
    if (audioFiles.length === 0) {
        showAdminToast('⚠️ Selecione pelo menos um arquivo de áudio!', "warning");
        return;
    }
    ...
    try {
        ...
        if (successfulTracks.length === 0) {
            showAdminToast('❌ Nenhuma música foi processada com sucesso!', "success", "success");
            btn.innerHTML = originalText;
            btn.disabled = false;
            return;
        }
        ...
        // Fechar modal ao concluir
        if(document.getElementById('addMusicModal')) {
        ...
        // Atualizar interface de edição de playlist (se estiver aberta)
        if (typeof window.playlistEditManager !== 'undefined' && window.playlistEditManager.currentPlaylistId === playlistId) {
...
async function calculateConstellationPreview() {
    ...
    try {
        ...
        if (locationType === 'manual') {
            ...
            const cachedLocation = localStorage.getItem('starMapUserLocation');
            if (cachedLocation) {
                try {
    ...
    try {
        ...
        if (locationType === 'manual') {
            ...
            if (isNaN(lat) || isNaN(lng)) {
                showAdminToast('⚠️ Digite coordenadas válidas!', "warning");
                btn.disabled = false;
                btn.innerHTML = originalText;
                return;
            }
       ...
       btn.disabled = false;
        ...
        if (typeof window.forceReloadStarMapConfig === 'function') {
            try {
                ...
                if (reloaded) {
                    showAdminToast('✅ Configurações salvas e Star Map atualizado automaticamente!', "success", "success");
                    console.log('✅ Star Map atualizado via preloader!');
                } else {
                    showAdminToast('✅ Configurações salvas!\n\n⚠️ Atualize a página para ver as mudanças.', "success", "success");
                }
            } catch (reloadError) {
...
console.log('✅ Módulo admin carregado — Retrospectiva gerenciada por retrospective.js');

// =========================================================================
//  HELPERS DE EXIF — usados pelo upload de fotos
// =========================================================================

/**
 * Converte coordenadas GPS DMS → graus decimais.
 * exifr pode entregar array [graus, min, seg] ou já decimal.
 */
...
/**
 * Geocodificação reversa via Nominatim (OSM) — sem chave API.
 * Retorna nome amigável do lugar em pt-BR.
 */
async function _reverseGeocode(lat, lng) {
    try {
        const url = `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=14&accept-language=pt-BR`;
        ...
        const parts = [
            a.attraction || a.leisure || a.tourism ||
        ...
        console.warn('Reverse geocode falhou:', e.message);
        return '';
    }
}
// ===== ADMIN ALBUMS UI (9:16 Grid) =====
async function renderAdminAlbums() {
    const grid = document.getElementById('adminAlbumsGrid');
    if (!grid) return;
    
    grid.innerHTML = '<div class="retro-admin-loading"><i class="fas fa-spinner fa-spin"></i> Carregando álbuns...</div>';
    ...
    try {
        ...
        if (snapshot.empty) {
            grid.innerHTML = '<p style="color: var(--theme-text-secondary); width: 100%; text-align: center;">Nenhum álbum encontrado.</p>';
            return;
        }
        ...
        snapshot.forEach(doc => {
            const album = doc.data();
            const id = doc.id;
            const cover = album.coverThumb || album.cover || 'assets/images/placeholder.jpg';
            const photoCount = album.photoCount || 0;
            const date = album.date || '';
            const title = album.title || 'Sem título';
            ...
            card.innerHTML = `
                <div class="admin-album-card-img-container">
                    <img src="${cover}" alt="${title}">
                </div>
                <div class="admin-album-card-overlay">
                    <h4 class="admin-album-card-title">${title}</h4>
                    <div class="admin-album-card-meta">
                        <span><i class="fas fa-calendar-alt"></i> ${date}</span>
                        <span><i class="fas fa-images"></i> ${photoCount} fotos</span>
                    </div>
                    <div class="admin-album-card-actions">
                        <div class="card-action-row">
                            <button class="admin-album-action-btn edit" onclick="editAlbumFromGrid('${id}')">
                                <i class="fas fa-pen"></i> Editar
                            </button>
                            <button class="admin-album-action-btn photos" onclick="addPhotosToAlbumFromGrid('${id}', '${safeTitle}')">
                                <i class="fas fa-plus"></i> Fotos
                            </button>
                        </div>
                        <button class="admin-album-action-btn delete" onclick="deleteAlbumFromGrid('${id}', '${safeTitle}')">
                            <i class="fas fa-trash"></i> Excluir
                        </button>
                    </div>
                </div>
        ...
        console.error('Erro ao renderizar álbuns:', error);
        grid.innerHTML = '<p style="color: #ff5050;">Erro ao carregar álbuns.</p>';
    }
}

function editAlbumFromGrid(id) {
    const modal = document.getElementById('editAlbumModal');
    if (modal) modal.style.display = 'flex';
    
    if (typeof loadAlbumForEdit === 'function') {
        loadAlbumForEdit(null, id);
    }
}

function addPhotosToAlbumFromGrid(id, title) {
    ...
    const nameEl = document.getElementById('addPhotosAlbumName');
    if (nameEl) nameEl.textContent = 'Adicionando fotos ao álbum: ' + title;
    
    document.getElementById('addPhotosModal').style.display = 'flex';
}

async function deleteAlbumFromGrid(id, title) {
    const isConfirmed = await window.showCustomConfirm('Excluir Álbum', `Tem certeza que deseja excluir o álbum "${title}" e TODAS as suas fotos? Esta ação não pode ser desfeita.`);
    if (isConfirmed) {
        try {
            ...
            showAdminToast('Álbum excluído com sucesso.', 'success');
            renderAdminAlbums();
            if (typeof loadAlbumsFromFirebase === 'function') loadAlbumsFromFirebase();
        } catch(e) {
            showAdminToast('Erro ao excluir: ' + e.message, 'error');
        }
    }
}

function initAlbumModals() {
    if (typeof updateAlbumSelect === 'function') updateAlbumSelect();

    const closeEdit = document.getElementById('closeEditAlbumModalBtn');
    if (closeEdit) {
        closeEdit.addEventListener('click', () => {
            if (typeof isInSelectionMode === 'function' && isInSelectionMode()) {
                cancelSelection();
                return;
            }
    ...
    const closePhotos = document.getElementById('closeAddPhotosModalBtn');
    if (closePhotos) {
    ...
    const openNewAlbum = document.getElementById('openNewAlbumModalBtn');
    if (openNewAlbum) {
        openNewAlbum.addEventListener('click', () => {
            document.getElementById('newAlbumModal').style.display = 'flex';
        });
    }

    const closeNewAlbum = document.getElementById('closeNewAlbumModalBtn');
    if (closeNewAlbum) {
        closeNewAlbum.addEventListener('click', () => {
            document.getElementById('newAlbumModal').style.display = 'none';
        });
    }

    // === Add Photos: File Input and Drop Zone ===
    ...
    if (photoDropZone) {
        ...
        ['dragenter', 'dragover'].forEach(eventName => {
            photoDropZone.addEventListener(eventName, () => photoDropZone.classList.add('dragover'), false);
        });

        ['dragleave', 'drop'].forEach(eventName => {
            photoDropZone.addEventListener(eventName, () => photoDropZone.classList.remove('dragover'), false);
        });
    ...
    // Refresh grid when admin modal opens
    const adminTabAlbums = document.querySelector('.admin-tab[data-tab="albums"]');
    if (adminTabAlbums) {
        adminTabAlbums.addEventListener('click', () => {
            renderAdminAlbums();
        });
    }
}
