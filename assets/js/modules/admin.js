    ...
    if (selectedCount > 0) {
        if (!isSelectionStatePushed) {
            if (typeof HistoryManager !== 'undefined') {
                HistoryManager.push('selection-mode');
            }
        ...
        if (isSelectionStatePushed && typeof HistoryManager !== 'undefined') {
            HistoryManager.remove('selection-mode');
        }
...
function setupBackButtonHandler() {
    window.addEventListener('popstate', (e) => {
        if (typeof isInSelectionMode === 'function' && isInSelectionMode()) {
            // Cancel selection but keep modal open
            cancelSelection();
        } else {
            // Close the top-most open modal only
            const modals = ['addPhotosModal', 'newAlbumModal', 'editAlbumModal'];
