...
// ===== CONFIGURAÇÃO DA RASPADINHA DO AMOR =====
async function initScratchCardAdminTab() {
    console.log('🔄 Inicializando aba Raspadinha do Amor...');
    const messageInput = document.getElementById('scratchCardMessage');
    const saveBtn = document.getElementById('scratchCardSaveBtn');

    if (!messageInput || !saveBtn) return;

    // Garante que listener não seja duplicado em re-aberturas
    if (saveBtn.dataset.listenerAttached === 'true') return;
    saveBtn.dataset.listenerAttached = 'true';

    // Carregar dados atuais do Firebase
    try {
        const doc = await db.collection('scratch_card_config').doc('settings').get();
        if (doc.exists && doc.data().message) {
            messageInput.value = doc.data().message;
        }
    } catch (e) {
        console.error('Erro ao carregar conf da Raspadinha', e);
    }

    // Salvar ao clicar no botão (sem submit de form — sem risco de reload)
    saveBtn.addEventListener('click', async () => {
        ...
        try {
            await db.collection('scratch_card_config').doc('settings').set({
                message: messageInput.value
            }, { merge: true });

            saveBtn.innerHTML = '<i class="fas fa-check"></i> Salvo com sucesso!';
            saveBtn.style.background = 'var(--theme-success, #27ae60)';
            ...
            saveBtn.style.background = 'var(--theme-error, #e74c3c)';
...
// Hook na inicialização das tabs — compatível com tabs já no DOM ao carregar
function hookScratchCardTab() {
    const adminTabs = document.querySelectorAll('.admin-tab');
    adminTabs.forEach(tab => {
        if (tab.dataset.scratchHookAttached === 'true') return;
        tab.dataset.scratchHookAttached = 'true';

        tab.addEventListener('click', () => {
            if (tab.dataset.tab === 'scratch-card') {
                // Sempre recarrega a mensagem ao entrar na aba.
                // O guard interno do botão (data-listener-attached) evita duplicar o listener de click.
                initScratchCardAdminTab();
            }
        });
    });
}

// Tenta no DOMContentLoaded; se o DOM já carregou (script carregado tarde), chama direto
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', hookScratchCardTab);
} else {
    hookScratchCardTab();
}