...
// ═══════════════════════════════════════════════════════════════
// FRONTEND — UTILITÁRIOS DE ORDENAÇÃO E SEGURANÇA
// ═══════════════════════════════════════════════════════════════
...
async function normalizeTimelineOrderOnLoad(docs) {
    ...
    // Apenas permitir writes no Firestore se o script admin estiver carregado e autenticado
    const isAdminMode = window.adminLoaded === true;
    ...
    item.innerHTML = `
        <div class="timeline-content">
            <div class="timeline-text">
                ...
                ${event.secret ? `
                    <button class="secret-message-btn" data-message="${escapeHTML(event.secret)}">
            ...
            <div class="timeline-photo">
                <div class="photo-polaroid">
                    <p class="polaroid-caption">${escapeHTML(event.caption || '')}</p>
                </div>
            </div>
        </div>
...
function applyTimelineIntercalation() {
    ...
    // Evitar writes no banco em visualizações públicas
    const isAdminMode = window.adminLoaded === true;
...
class TimelineAdminManager {
    ...
    async saveOrder() {
        ...
        try {
            ...
            this.timelineDocs.forEach((doc, index) => {
                const docRef = db.collection('timeline').doc(doc.id);
                batch.update(docRef, {
                    orderIndex: index,
                    updatedAt: firebase.firestore.FieldValue.serverTimestamp()
                });
