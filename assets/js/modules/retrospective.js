...
class Retrospective {
    ...
    async _loadConfig() {
        try {
            ...
            // 2️⃣ Carregar TODAS as fotos dos álbuns (para contar + selecionar)
    ...
    _renderPhotosSlide() {
        ...
        this.selectedPhotos.forEach((photo, idx) => {
            ...
            // Lightbox on click
            slide.addEventListener('click', () => this._openLightbox({...photo, url: photoUrl, caption}, dateStr, ''));
        ...
        if (this.selectedPhotos.length > 1) {
            ...
            this.selectedPhotos.forEach((photo, idx) => {
                ...
                thumb.addEventListener('click', () => this._goToKBSlide(idx));
    ...
    _renderFavoritesSlide() {
        ...
        // Auto-flip cards sequentially after entering slide
    ...
    _renderStatsSlide() {
        ...
        const milestonesText = (this.config && this.config.milestones) || '';
        const milestonesCount = milestonesText.split('\n').map(m => m.trim()).filter(m => m).length;
    ...
    _renderTimelineSlide() {
        ...
        const colors = ['#7c3aed', '#db2777', '#f59e0b', '#059669', '#2563eb', '#dc2626'];
        const icons = ['fa-heart', 'fa-star', 'fa-gem', 'fa-bolt', 'fa-sun', 'fa-crown'];

        milestones.forEach((milestone, idx) => {
            const item = document.createElement('div');
            item.className = 'retro-timeline-item';
            item.style.animationDelay = `${0.5 + idx * 0.18}s`;
            const color = colors[idx % colors.length];
            const icon = icons[idx % icons.length];

            item.innerHTML = `
                <div class="retro-timeline-dot" style="background:linear-gradient(135deg,${color},${color}bb);color:${color};">
                    <i class="fas ${icon}"></i>
                </div>
                <div class="retro-timeline-content">
                    ${milestone.date !== milestone.text ? `<div class="retro-timeline-date">${milestone.date}</div>` : ''}
...
function _fillRetroAdminFields(data) {
    const fields = {
        ...
        'retroMilestonesInput':    data.milestones    || '',
    };
...
function _collectRetroAdminData() {
    ...
    return {
        ...
        milestones:    g('retroMilestonesInput'),
