...
class Retrospective {
    constructor() {
        ...
        this.slideThemes = [
            ...
            // Slide 3: Timeline — amber/emerald
            [{ r: 180, g: 100, b: 10 }, { r: 10, g: 150, b: 100 }],
            // Slide 4: Favorites — gold/purple
            [{ r: 180, g: 120, b: 20 }, { r: 120, g: 40, b: 200 }],
            // Slide 5: Final — deep pink/purple (grand finale)
    ...
    _bindEvents() {
        ...
        // Evento de Voltar do Android/iOS (History API)
        window.addEventListener('popstate', (e) => {
            if (this.modal && this.modal.style.display !== 'none') {
                if (e.state && e.state.retroFavFocus) return; // Permanece aberto, o history.back do focus vai fechar
                
                if (this._closeFocusMode) {
                    const closed = this._closeFocusMode(true); // true indica que veio do popstate
                    if (closed) return;
                }

                if (e.state && e.state.retroModal) return; // Se a URL tiver hash, o modal fica aberto
                this._isClosingFromPopState = true;
                this.closeModal();
            }
        });
        ...
        // Swipe para baixo na parte superior do modal para fechar (1/3 superior)
        if (this.modal) {
            let swipeStartY = 0;
            let swipeStartX = 0;
            ...
            this.modal.addEventListener('touchstart', (e) => {
                // Só ativa o swipe-to-close se tocar no terço superior da tela
                if (e.touches[0].clientY > window.innerHeight / 3) return;
            ...
            this.modal.addEventListener('touchmove', (e) => {
                ...
                // Só arrasta para baixo e ignora swipe horizontal
                if (dy > 0 && Math.abs(dy) > Math.abs(dx)) {
                    // Modal acompanha o dedo suavemente (efeito elástico leve no fim)
                    this.modal.style.transition = 'none';
                    this.modal.style.transform = `translateY(${dy}px)`;
                    this.modal.style.opacity = `${Math.max(1 - dy / (window.innerHeight * 0.8), 0.2)}`;
                }
            }, { passive: true });
            ...
            this.modal.addEventListener('touchend', (e) => {
                ...
                // Se arrastou mais de 20% da tela para baixo, fecha
                if (dy > window.innerHeight * 0.2 && Math.abs(dy) > Math.abs(dx)) {
                    this.modal.style.transition = 'transform 0.4s cubic-bezier(0.22,1,0.36,1), opacity 0.3s ease';
                    this.modal.style.transform = 'translateY(100%)';
                    this.modal.style.opacity = '0';
                    setTimeout(() => this.closeModal(), 400);
                } else {
                    // Snap de volta
                    this.modal.style.transition = 'transform 0.35s cubic-bezier(0.22,1,0.36,1), opacity 0.35s ease';
    ...
    // =========================================================================
    async openModal() {
        // Desativa restauração automática do browser para controlarmos manualmente
        if ('scrollRestoration' in history) history.scrollRestoration = 'manual';
        // Salvar posição do scroll em variável própria sem sobrescrever o sistema do menu.
        // Se o menu estava aberto, ele já tem o valor correto em window.menuScrollPosition.
        // Caso contrário, usa o scrollY atual.
        window.retroScrollPosition = (window.menuScrollPosition !== undefined && document.body.classList.contains('menu-open'))
            ? window.menuScrollPosition
            : (window.scrollY || document.documentElement.scrollTop);
        this.modal.style.display = 'flex';
        document.body.style.overflow = 'hidden';

        // History API para botão voltar do celular
        if (!this._isOpeningFromPopState) {
            try {
                history.pushState({ retroModal: true }, '', '#retrospectiva');
            } catch (e) {
                console.warn('History API indisponível', e);
            }
        }
        this._isOpeningFromPopState = false;
    ...
    closeModal() {
        // Resetar qualquer transform/opacity do swipe-to-close
        ...
        // History API para limpar a URL caso fechado manualmente (X, swipe ou ESC)
        // Usa replaceState em vez de history.back() para não disparar popstate e não resetar o scroll
        if (!this._isClosingFromPopState) {
            if (window.location.hash === '#retrospectiva' || window.location.hash === '#retrospectiva-favorito') {
                try {
        ...
        // Restaurar scroll para onde estava antes de abrir a retrospectiva
        // scrollRestoration = 'manual' garante que o browser não sobrescreve o nosso scrollTo
        if (window.retroScrollPosition !== undefined) {
            document.documentElement.style.scrollBehavior = 'auto';
            window.scrollTo(0, window.retroScrollPosition);
            document.documentElement.style.scrollBehavior = '';
        }
        // Reativa restauração automática do browser
        if ('scrollRestoration' in history) history.scrollRestoration = 'auto';
        this._isClosingFromPopState = false;
        this._stopStars();
        this._stopBlobs();
        this._stopConfetti();
        this._stopKenBurns();
    ...
    async _loadConfig() {
        ...
        try {
            ...
            console.warn('⚠️ Retrospectiva: erro ao carregar timeline', e);
        }
        ...
        try {
            ...
            console.warn('⚠️ Retrospectiva: erro ao carregar fotos', e);
        }
    }
    ...
    async _loadAndSelectPhotosIntelligently() {
        ...
        try {
            ...
            const pagesPromises = sortedAlbums.map(albumDoc => 
                db.collection('album_photos')
                  .where('albumId', '==', albumDoc.id)
                  .get()
                  .then(snap => ({ albumDoc, pagesSnap: snap }))
            );
            
            const pagesResults = await Promise.all(pagesPromises);

            for (const { albumDoc, pagesSnap } of pagesResults) {
    ...
    async _startSlides() {
        ...
        let originalText = '';
        if (startBtn) {
            originalText = startBtn.innerHTML;
            startBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Preparando...';
            startBtn.style.opacity = '0.7';
            startBtn.style.pointerEvents = 'none';
            try { await this._configPromise; } catch(e) {}
    ...
    _renderPhotosSlide() {
        ...
        this.selectedPhotos.forEach((photo, idx) => {
            ...
            // Navegação por toque dentro da foto (metade esquerda = volta, direita = avança)
            let tapStartX = 0;
            let tapStartY = 0;
            ...
            slide.addEventListener('touchend', (e) => {
                ...
                const dx = tapEndX - tapStartX;
                const dy = tapEndY - tapStartY;

                // Ignorar se foi um swipe (movimento > 12px)
                if (Math.abs(dx) > 12 || Math.abs(dy) > 12) return;

                const rect = slide.getBoundingClientRect();
                const x = tapEndX - rect.left;
                if (x < rect.width / 2) {
            ...
            // Fallback click para mouse/desktop
    ...
    _goToKBSlide(index) {
        ...
        thumbs.forEach((t, i) => {
            ...
            if (i === index) {
                // Removemos o scrollIntoView pois ele causa bug de deslocamento de tela no mobile quando há transform: translateX
                if (this.thumbStrip) {
                    const scrollLeftTarget = t.offsetLeft - (this.thumbStrip.clientWidth / 2) + (t.clientWidth / 2);
                    this.thumbStrip.scrollTo({ left: scrollLeftTarget, behavior: 'smooth' });
                }
            }
        });
        ...
        // Se estiver rodando no auto-play, reseta o tempo para não pular logo em seguida
        if (this.kbInterval) {
            this._startKenBurns();
        }
    }
    ...
    _renderFavoritesSlide() {
        ...
        items.forEach((item, i) => {
            ...
            // Novo comportamento: Focus Mode em vez de Flip no click
            card.addEventListener('click', () => {
                if (!card.classList.contains('flipped')) return; // Só abre se já estiver revelado
                if (document.querySelector('.retro-fav-card-focus-clone')) return; // Previne múltiplos cliques
                
                let overlay = document.querySelector('.retro-focus-overlay');
                if (!overlay) {
                    overlay = document.createElement('div');
                    overlay.className = 'retro-focus-overlay';
                    document.body.appendChild(overlay);
                }
                
                const rect = card.getBoundingClientRect();
                const clone = card.cloneNode(true);
                clone.className = 'retro-fav-card retro-fav-card-focus-clone flipped';
                ...
                // History API: Registrar estado do Focus Mode
                try {
                    history.pushState({ retroFavFocus: true, retroModal: true }, '', '#retrospectiva-favorito');
                } catch (e) {
                    // Ignora erro no file:// protocol
                }
                ...
                const closeFocus = (fromPopState = false) => {
                    if (!document.body.contains(clone)) return false;
                    ...
                    // Limpar URL se não foi fechado pelo botão voltar do celular
                    if (!fromPopState && window.location.hash === '#retrospectiva-favorito') {
                        try {
                            history.back();
                        } catch (e) {}
                    }
                    
                    this._closeFocusMode = null;
                    return true;
                };

                this._closeFocusMode = closeFocus;
                overlay.onclick = () => closeFocus(false);
                clone.onclick = () => closeFocus(false);
            });
    ...
    _renderStatsSlide() {
        ...
        const stats = [
            ...
            { icon: '🏆', number: milestonesCount, label: 'Marcos especiais', delay: 0.5 },
            { icon: '💜', number: daysCount, label: 'Dias de amor', delay: 0.7 },
        ];
    ...
    _showSlide(index, direction = 'enter') {
        ...
        // Reset flip cards when entering favorites (slide 4)
        if (index === 4) {
    ...
    _initBlobs() {
        ...
        resize();
        this._blobResizeHandler = resize;
        window.addEventListener('resize', this._blobResizeHandler);
    ...
    _stopBlobs() {
        ...
        if (this._blobResizeHandler) {
            window.removeEventListener('resize', this._blobResizeHandler);
            this._blobResizeHandler = null;
        }
    }
    ...
    _initStars() {
        ...
        resize();
        this._starResizeHandler = resize;
        window.addEventListener('resize', this._starResizeHandler);
    ...
    _stopStars() {
        ...
        if (this._starResizeHandler) {
            window.removeEventListener('resize', this._starResizeHandler);
            this._starResizeHandler = null;
        }
...
async function _loadAlbumPhotosForAdmin() {
    ...
    try {
        if (typeof db !== 'undefined') {
            ...
            const pagesPromises = snap.docs.map(albumDoc =>
                db.collection('album_photos')
                  .where('albumId', '==', albumDoc.id)
                  .get()
                  .then(photoPagesSnap => ({ albumDoc, photoPagesSnap }))
            );
            
            const results = await Promise.all(pagesPromises);
            
            for (const { albumDoc, photoPagesSnap } of results) {
...
async function _loadPlaylistsForAdmin() {
    ...
    try {
        ...
        if (musicsFromPlaylists.length === 0 && typeof db !== 'undefined') {
            ...
            const tracksPromises = playlistsSnap.docs.map(playlistDoc => 
                db.collection('playlist_tracks')
                  .where('playlistId', '==', playlistDoc.id)
                  .get()
                  .then(tracksSnap => ({ playlistDoc, tracksSnap }))
            );

            const tracksResults = await Promise.all(tracksPromises);

            for (const { playlistDoc, tracksSnap } of tracksResults) {
