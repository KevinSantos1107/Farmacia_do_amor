...
const WordGame = {
    ...
    async loadWordsFromFirebase() {
        ...
        try {
            ...
            const snapshot = await db.collection('word_game').orderBy('createdAt', 'asc').limit(50).get();
    ...
    restoreGame() {
        ...
        try {
            const rawState = localStorage.getItem('wordGameState');
            ...
            console.warn('Erro ao restaurar jogo do localStorage', e);
            this.clearGameState();
            this.startNewGame();
        }
    },
        ...
        try {
            ...
            if (saved) {
                try {
                    ...
                    if (!Array.isArray(this.usedWords)) this.usedWords = [];
                } catch(e) {
                    this.usedWords = [];
                }
            } else {
    ...
    shareOnWhatsApp() {
        ...
        // Pega a URL atual do site para convidar a pessoa a jogar
        // Limpa possíveis parâmetros ou âncoras na URL
