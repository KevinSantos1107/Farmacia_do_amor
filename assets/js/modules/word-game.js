...
const WordGame = {
    ...
    async loadDictionary() {
        try {
            console.log('📥 Carregando dicionário de palavras...');
            const response = await fetch('./assets/data/pt-5-letters.json');
            if (response.ok) {
                const words = await response.json();
                this.validWords = new Set(words);
                console.log(`✅ Dicionário carregado: ${this.validWords.size} palavras`);
            } else {
                console.warn('⚠️ Falha ao carregar dicionário (Status:', response.status, ')');
            }
        } catch (error) {
    ...
    submitGuess() {
        ...
        // ✨ NOVO: Verifica se a palavra existe no dicionário
        if (this.wordLength === 5 && this.validWords.size > 0) {
