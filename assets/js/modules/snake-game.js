...
class SnakeGame {
    ...
    bindSwipe() {
        ...
        this.modal.addEventListener('touchmove', e => {
            ...
            if (Math.abs(dx) > 25 || Math.abs(dy) > 25) {
                ...
                // Atualiza a âncora para permitir curvas sucessivas sem levantar o dedo
                sx = cx;
                sy = cy;
            }
        }, { passive: true });
    ...
    resizeCanvas() {
        ...
        // Remove estilos inline de tamanho para que o CSS controle livremente
        this.canvas.style.width  = '';
        this.canvas.style.height = '';

        // Lê as dimensões reais renderizadas pelo browser APÓS o layout CSS ser aplicado.
        // Isso é crucial no iPhone, onde Safari pode aplicar zoom, safe areas ou
        // ajustes de viewport que tornam clientWidth diferente do tamanho real do canvas.
        const rect = this.canvas.getBoundingClientRect();
        const cssW = rect.width;
        const cssH = rect.height;

        // Calcula CELL a partir das dimensões reais, usando o menor valor para garantir
        // que COLS×CELL e ROWS×CELL nunca excedam o espaço disponível (sem overflow).
        const cellFromW = cssW / this.COLS;
        const cellFromH = cssH / this.ROWS;
        this.CELL = Math.floor(Math.min(cellFromW, cellFromH));

        // Calcula o offset para centralizar o tabuleiro dentro do canvas.
        // A margem residual (< 1px por célula) é distribuída igualmente dos dois lados.
        this.offsetX = Math.floor((cssW - this.COLS * this.CELL) / 2);
        this.offsetY = Math.floor((cssH - this.ROWS * this.CELL) / 2);

        // Configura o buffer físico do canvas com resolução total (dpr correto).
        const dpr = window.devicePixelRatio || 1;
        this.canvas.width  = Math.round(cssW * dpr);
        this.canvas.height = Math.round(cssH * dpr);

        // Reseta e escala o contexto para que 1 unidade = 1 CSS pixel.
    ...
    startGame() {
        ...
        // Spawna blocos fixos do nível ANTES da comida, para não nascer parede na comida
        const cfg = this.LEVELS[this.difficulty];
    ...
    randomFreeCell(isHazard = false) {
        ...
        for (let x = 0; x < this.COLS; x++) {
            for (let y = 0; y < this.ROWS; y++) {
                ...
                validRelaxed.push({x, y});

                // Evita armadilhas: a célula deve ter no mínimo 2 saídas livres
                if (this.getFreeNeighborsCount(x, y) >= 2) {
                    validStrict.push({x, y});
                }
            }
        }

        let choices = validStrict.length > 0 ? validStrict : 
                     (validRelaxed.length > 0 ? validRelaxed : validAny);

        if (choices.length === 0) return {x: 0, y: 0}; // map full fallback
    ...
    draw(progress) {
        ...
        // Limpa toda a área física do canvas (incluindo margens do offset)
        ctx.clearRect(0, 0, this.canvas.width / (window.devicePixelRatio || 1), this.canvas.height / (window.devicePixelRatio || 1));

        // Aplica offset para centralizar o tabuleiro no canvas.
        // Todos os elementos (grade, cobra, itens) compartilham este mesmo sistema de coordenadas.
        ctx.save();
        ctx.translate(this.offsetX, this.offsetY);

        // Fundo xadrez
        ctx.clearRect(0, 0, this.COLS * C, this.ROWS * C);
        ...
        ctx.restore(); // restaura o sistema de coordenadas (remove o translate do offset)
    }
    ...
    drawIdle() {
        ...
        // Limpa toda a área física do canvas
        ctx.clearRect(0, 0, this.canvas.width / (window.devicePixelRatio || 1), this.canvas.height / (window.devicePixelRatio || 1));

        // Aplica o mesmo offset que draw() para consistência total
        ctx.save();
        ctx.translate(this.offsetX, this.offsetY);
        ...
        ctx.lineWidth   = 0.5;
        for (let c = 0; c <= this.COLS; c++) {
            ctx.beginPath(); ctx.moveTo(c * C, 0); ctx.lineTo(c * C, this.ROWS * C); ctx.stroke();
        }
        for (let r = 0; r <= this.ROWS; r++) {
            ctx.beginPath(); ctx.moveTo(0, r * C); ctx.lineTo(this.COLS * C, r * C); ctx.stroke();
        }
