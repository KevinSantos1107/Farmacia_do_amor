...
async function precalculateConstellations() {
    try {
        ...
        if (window.starMapState.config && window.starMapState.config.customLocation) {
            ...
            // Apenas usar localização se já estiver salva em cache, para não pedir permissão no preloader
            ...
            if (cachedLocation) {
                try {
                    ...
                    console.log('📍 Localização recuperada do cache no preloader:', latitude, longitude);
                } catch (e) {
                    latitude = 42.4164;
                    longitude = -88.6137;
                    console.log('📍 Cache inválido, usando localização padrão');
                }
            } else {
                // Fallback: Harvard, Illinois
                latitude = 42.4164;
                longitude = -88.6137;
                console.log('📍 Usando localização padrão (Harvard, IL) para o preloader');
            }
        }
