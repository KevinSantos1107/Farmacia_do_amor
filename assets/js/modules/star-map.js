...
const CONSTELLATIONS_DATA = {
    'Cassiopeia': { stars: [{x: 520, y: 180}, {x: 420, y: 260}, {x: 540, y: 320}, {x: 440, y: 400}, {x: 560, y: 460}], season: 'outono', hemisphere: 'norte', magnitude: 9 },
    'Orion': { stars: [{x: 200, y: 300}, {x: 140, y: 400}, {x: 240, y: 480}, {x: 180, y: 580}], season: 'inverno', hemisphere: 'both', magnitude: 10 },
    'Crux': { stars: [{x: 460, y: 560}, {x: 380, y: 660}, {x: 440, y: 720}, {x: 520, y: 640}], season: 'ano_todo', hemisphere: 'sul', magnitude: 10 },
    'Ursa Major': { stars: [{x: 600, y: 200}, {x: 650, y: 250}, {x: 680, y: 300}, {x: 650, y: 350}, {x: 600, y: 380}, {x: 550, y: 340}, {x: 530, y: 280}], season: 'primavera', hemisphere: 'norte', magnitude: 9 },
    'Leo': { stars: [{x: 300, y: 200}, {x: 350, y: 240}, {x: 400, y: 220}, {x: 380, y: 280}, {x: 320, y: 300}], season: 'primavera', hemisphere: 'both', magnitude: 8 },
    'Scorpius': { stars: [{x: 150, y: 500}, {x: 200, y: 540}, {x: 250, y: 580}, {x: 280, y: 620}, {x: 300, y: 670}], season: 'verao', hemisphere: 'both', magnitude: 9 },
    'Aquarius': { stars: [{x: 500, y: 450}, {x: 540, y: 500}, {x: 580, y: 540}, {x: 560, y: 590}], season: 'outono', hemisphere: 'both', magnitude: 7 },
    'Taurus': { stars: [{x: 250, y: 250}, {x: 300, y: 280}, {x: 340, y: 320}, {x: 320, y: 370}], season: 'inverno', hemisphere: 'both', magnitude: 8 },
    'Gemini': { stars: [{x: 350, y: 180}, {x: 400, y: 160}, {x: 420, y: 210}, {x: 380, y: 250}, {x: 340, y: 240}], season: 'inverno', hemisphere: 'norte', magnitude: 7 },
    'Virgo': { stars: [{x: 450, y: 300}, {x: 490, y: 350}, {x: 520, y: 390}, {x: 480, y: 430}], season: 'primavera', hemisphere: 'both', magnitude: 8 },
    'Sagittarius': { stars: [{x: 180, y: 600}, {x: 220, y: 640}, {x: 260, y: 680}, {x: 240, y: 720}], season: 'verao', hemisphere: 'sul', magnitude: 9 },
    'Cygnus': { stars: [{x: 600, y: 450}, {x: 630, y: 500}, {x: 650, y: 550}, {x: 620, y: 600}, {x: 580, y: 580}], season: 'verao', hemisphere: 'norte', magnitude: 8 },
    'Andromeda': { stars: [{x: 400, y: 100}, {x: 450, y: 150}, {x: 500, y: 180}, {x: 550, y: 160}], season: 'outono', hemisphere: 'norte', magnitude: 8 },
    'Pegasus': { stars: [{x: 300, y: 400}, {x: 380, y: 400}, {x: 380, y: 480}, {x: 300, y: 480}, {x: 300, y: 400}], season: 'outono', hemisphere: 'both', magnitude: 8 },
    'Draco': { stars: [{x: 700, y: 300}, {x: 750, y: 350}, {x: 720, y: 400}, {x: 680, y: 380}, {x: 650, y: 420}], season: 'verao', hemisphere: 'norte', magnitude: 7 },
    'Hercules': { stars: [{x: 500, y: 600}, {x: 550, y: 580}, {x: 580, y: 620}, {x: 530, y: 650}, {x: 500, y: 600}], season: 'verao', hemisphere: 'both', magnitude: 7 },
    'Ursa Minor': { stars: [{x: 400, y: 300}, {x: 430, y: 320}, {x: 460, y: 350}, {x: 440, y: 380}], season: 'ano_todo', hemisphere: 'norte', magnitude: 8 },
    'Centaurus': { stars: [{x: 200, y: 700}, {x: 250, y: 750}, {x: 300, y: 720}, {x: 350, y: 780}], season: 'primavera', hemisphere: 'sul', magnitude: 9 },
    'Canis Major': { stars: [{x: 100, y: 400}, {x: 150, y: 450}, {x: 120, y: 500}, {x: 80, y: 480}], season: 'inverno', hemisphere: 'both', magnitude: 9 },
    'Lyra': { stars: [{x: 600, y: 600}, {x: 630, y: 650}, {x: 680, y: 630}, {x: 650, y: 580}, {x: 600, y: 600}], season: 'verao', hemisphere: 'norte', magnitude: 8 }
};
...
function selectVisibleConstellations(date, latitude, longitude) {
    ...
    // Selecionar até 10 constelações (dependendo de quantas derem match perfeito)
    const maxConstellations = 10;
    const selected = available.slice(0, Math.min(available.length, maxConstellations));
    
    console.log(`✨ ${selected.length} constelações selecionadas para ${date.toLocaleDateString()}:`);
...
function redistributeConstellations(constellations) {
    ...
    // Distribuir dinamicamente em círculos ao redor do centro
    return constellations.map(([name, data], index) => {
        const total = constellations.length;
        // Se for 1 fica no centro, senão distribui em anel
        const angleRad = total > 1 ? (index * (Math.PI * 2)) / total : 0;
        
        // Varia um pouco o raio para não ficar um anel perfeito
        const currentRadius = baseRadius * (0.6 + Math.random() * 0.6);
        
        const offsetX = Math.sin(angleRad) * currentRadius;
        const offsetY = -Math.cos(angleRad) * currentRadius;
        ...
        const redistributedStars = data.stars.map(star => {
            ...
            // Reduzir tamanho um pouco para caber várias constelações
            const scale = total > 5 ? 0.5 : 0.8;
            
            return {
                x: centerX + offsetX + relativeX * scale,
                y: centerY + offsetY + relativeY * scale
            };
        });
        
        console.log(`   📍 ${name} reposicionada`);
...
function initStarMapModal() {
    ...
    // ===== FULLSCREEN API =====
    const fullscreenBtn = document.getElementById('fullscreenStarMapBtn');
    if (fullscreenBtn) {
...
class StarMap {
    constructor(constellations) {
        ...
        this.stopWarpButton = document.getElementById('stopStarWarp');
        ...
        this.loveMessages = [
            ...
            { depth: 54, text: "사랑해", lang: "Coreano", color: "#ffa8d8" },
            { depth: 61, text: "Я тебя люблю", lang: "Russo", color: "#ff69b4" },
            { depth: 68, text: "Σ' αγαπώ", lang: "Grego", color: "#ff9ed5" },
            { depth: 75, text: "أحبك", lang: "Árabe", color: "#ffb3db" },
            { depth: 82, text: "मैं तुमसे प्यार करता हूँ", lang: "Hindi", color: "#ff1493" },
            { depth: 89, text: "Jag älskar dig", lang: "Sueco", color: "#ffc2e0" },
            { depth: 96, text: "Seni seviyorum", lang: "Turco", color: "#ff9ed5" },
            { depth: 103, text: "Ik hou van jou", lang: "Holandês", color: "#ffa8d8" }
        ];
        ...
        this.stopWarpButtonHandler = () => this.stopWarpEffect();
    ...
    setupParallax() {
        ...
        this.parallaxHandler = (e) => {
            if (!this.warpActive) {
                ...
                this.targetParallaxX = moveX * 20; // max 20px offset
                this.targetParallaxY = moveY * 20;
            }
        };
        
        this.gyroHandler = (e) => {
            if (!this.warpActive && e.gamma && e.beta) {
                // gamma is left/right, beta is front/back
                const moveX = e.gamma / 45; // roughly normalize
                const moveY = (e.beta - 45) / 45;
                
                this.targetParallaxX = Math.min(Math.max(moveX, -1), 1) * 20;
                this.targetParallaxY = Math.min(Math.max(moveY, -1), 1) * 20;
            }
        };

        modal.addEventListener('mousemove', this.parallaxHandler);
        modal.addEventListener('touchmove', this.parallaxHandler);
    ...
    init() {
        ...
        if (this.startWarpButton) {
            ...
            this.startWarpButton.style.display = 'flex';
        ...
        if (this.stopWarpButton) {
            this.stopWarpButton.style.display = 'none';
            this.stopWarpButton.addEventListener('click', this.stopWarpButtonHandler);
        }
    ...
    destroy() {
        ...
        if (this.stopWarpButton && this.stopWarpButtonHandler) {
            this.stopWarpButton.removeEventListener('click', this.stopWarpButtonHandler);
        }
        
        const modal = document.getElementById('starMapModal');
        if (modal && this.parallaxHandler) {
            modal.removeEventListener('mousemove', this.parallaxHandler);
            modal.removeEventListener('touchmove', this.parallaxHandler);
        }
        if (this.gyroHandler) {
...
initBackgroundStars() {
    ...
    startWarpEffect() {
        ...
        if (this.stopWarpButton) {
            this.stopWarpButton.style.display = 'flex';
        }
        console.log('🚀 Viagem warp iniciada!');
    }
    
    stopWarpEffect() {
        if (!this.warpActive || this.returningToStart) return;
        
        console.log('🛑 Sair da Viagem clicado, retornando ao mapa...');
    ...
    drawInitialAnimation(progress) {
        ...
        this.parallaxX += (this.targetParallaxX - this.parallaxX) * 0.1;
        this.parallaxY += (this.targetParallaxY - this.parallaxY) * 0.1;
        ...
        this.ctx.save();
        // Apply parallax to the whole star field
        this.ctx.translate(this.centerX + this.parallaxX, this.centerY + this.parallaxY);
        this.ctx.scale(1, 1);
    ...
    drawWarpSpeed() {
        ...
        // Efeito visual com as mensagens de amor em 3D e rotação
        this.loveMessages.forEach((msg, idx) => {
            const zPos = ((msg.depth - this.zoomDepth) % 100 + 100) % 100;
            
            if (zPos > 0 && zPos < 100) {
                const depthFactor = 100 / zPos;
                // Espiral: rotação baseada no índice e profundidade
                const angleOffset = (idx / this.loveMessages.length) * Math.PI * 2 + (this.zoomDepth * 0.05);
                const radiusOffset = 80; // Distância do centro
                
                const xOffset = Math.cos(angleOffset) * radiusOffset;
                const yOffset = Math.sin(angleOffset) * radiusOffset;
                
                const px = this.centerX + xOffset * depthFactor;
                const py = this.centerY + yOffset * depthFactor;
                
                const scale = Math.max(0.1, Math.min(5, depthFactor * 0.8));
                const textOpacity = Math.min(1, Math.max(0, (100 - zPos) / 40)) * (zPos < 10 ? zPos / 10 : 1) * particleOpacity;
                
                if (px > -100 && px < this.canvas.width + 100 && py > -100 && py < this.canvas.height + 100 && scale > 0.2) {
                    this.ctx.save();
                    ...
                    this.ctx.fillText(msg.text, 0, -10);
                    
                    this.ctx.font = '12px Poppins, sans-serif';
                    this.ctx.shadowBlur = 5;
                    this.ctx.fillStyle = `rgba(255, 255, 255, ${textOpacity * 0.7})`;
                    this.ctx.fillText(msg.lang, 0, 15);
