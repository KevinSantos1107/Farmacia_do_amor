    ...
    try {
        ...
        if (config && config.customLocation) {
            ...
            try {
                ...
                if (!window.starMapState) window.starMapState = {};
        ...
        if (window.starMapState && window.starMapState.constellations) {
            if (!window.starMapState.detectedLocation && !(config && config.customLocation) && (latitude !== 42.4164 || longitude !== -88.6137)) {
                needsRecalc = true;
            }
        } else {
            needsRecalc = true;
        ...
        if (!needsRecalc && window.starMapState && window.starMapState.constellations) {
            ...
            if (window.starMapState) {
                window.starMapState.constellations = visibleConstellations;
            }
        }
...
function getCurrentPosition() {
    return new Promise((resolve, reject) => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                localStorage.setItem('starMapUserLocation', JSON.stringify({
...
initBackgroundStars() {
    ...
    createParticle(forceNew = false) {
        const angle = Math.random() * Math.PI * 2;
        // Distribute stars in a wider radius
        const distance = (Math.random() * 800) + 50; 
        return {
            x: Math.cos(angle) * distance,
            y: Math.sin(angle) * distance,
            z: forceNew ? 1000 : Math.random() * 1000,
    ...
    drawInitialAnimation(progress, timestamp = 0) {
        this.ctx.setTransform(1, 0, 0, 1, 0, 0);
        ...
        const totalWaves = 5;
        const timeOffset = timestamp * 0.001; // For twinkling
        ...
        this.backgroundStars.forEach((star, index) => {
            ...
            let opacity = waveProgress;
            
            // Add continuous twinkle effect when animation is done
            if (progress >= 1) {
                const twinkleSpeed = 1.5 + (index % 3);
                const twinklePhase = index * 0.1;
                const twinkle = (Math.sin(timeOffset * twinkleSpeed + twinklePhase) * 0.3) + 0.7; // 0.4 to 1.0
                opacity *= twinkle;
            }
        ...
        this.constellations.forEach((constellation, constIndex) => {
            ...
            if (constellationProgress >= 1) {
                ...
                if (timeSinceComplete >= 0 && timeSinceComplete < this.pulseDuration) {
                    ...
                    const continuousTime = timestamp * 0.001;
                    const phase = constIndex * (Math.PI / 2);
                    pulseEffect = (Math.sin(continuousTime * 2 + phase) * 0.2) + 0.1;
                }
            }
    ...
    drawWarpSpeed() {
        ...
        if (transitionProgress < 1) {
            this.drawInitialAnimation(1, Date.now());
        ...
        // Add rotation effect to the warp
        this.ctx.save();
        this.ctx.translate(this.centerX, this.centerY);
        this.ctx.rotate(warpProgress * Math.PI * 0.5); // 90 degree rotation over the warp
        this.ctx.translate(-this.centerX, -this.centerY);
        
        this.particles.forEach((p, index) => {
            p.z -= this.zoomSpeed * 2000 * p.speed; // Faster Z speed
            ...
            // True 3D projection
            const scale = 400 / Math.max(1, p.z);
            ...
            const previousZ = p.z + this.zoomSpeed * 2000 * p.speed;
            const prevScale = 400 / Math.max(1, previousZ);
            const prevX = this.centerX + p.x * prevScale;
            const prevY = this.centerY + p.y * prevScale;
            
            // Don't draw if it's completely out of bounds to save performance
            const margin = 100;
            if (x2d < -margin || x2d > this.canvas.width + margin || y2d < -margin || y2d > this.canvas.height + margin) {
                return; 
            }
            
            const trailGradient = this.ctx.createLinearGradient(x2d, y2d, prevX, prevY);
            trailGradient.addColorStop(0, `rgba(255, 255, 255, ${p.brightness * particleOpacity})`);
            ...
            // Small bright head
            this.ctx.fillStyle = `rgba(255, 255, 255, ${p.brightness * particleOpacity})`;
            this.ctx.beginPath();
    ...
    drawScene(progress, timestamp = 0) {
        if (this.returningToStart) {
            ...
            this.drawInitialAnimation(progress, timestamp);
        } else {
            this.drawInitialAnimation(1, timestamp);
        }
    }
    ...
    animate(timestamp) {
        ...
        this.drawScene(Math.min(this.animationProgress, 1), timestamp);
