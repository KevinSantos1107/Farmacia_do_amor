...
class PlaylistEditManager {
                ...
                <div class="playlist-edit-container">
                    ...
                    <div class="playlist-edit-content">
                        ...
                        <div class="playlist-music-section">
                            <div class="music-section-header" style="display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <button type="button" id="addMusicToPlaylistBtn" title="Adicionar Música" style="background: rgba(0,255,128,0.15); border: 1px solid rgba(0,255,128,0.3); border-radius: 12px; width: 42px; height: 42px; color: #00ff80; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; transition: all 0.3s ease; flex-shrink: 0;">
    ...
    setupEventListeners() {
        ...
        // Listener do botão de adicionar música
        const addMusicBtn = document.getElementById('addMusicToPlaylistBtn');
        if (addMusicBtn) {
            addMusicBtn.addEventListener('click', (e) => {
                e.preventDefault();
                console.log('✅ Botão de adicionar música clicado!');
                if (typeof window.openAddMusicModalFromEdit === 'function') {
                    window.openAddMusicModalFromEdit();
                }
            });
        }
    ...
    async openModal(playlistId) {
        try {
            ...
            if(document.getElementById('playlistCoverPreview')) {
    ...
    async persistTracksToPages() {
        ...
        try {
            console.log('🗑️ Removendo tracks antigos e salvando novos via Batch...');
            ...
            // Usando Firestore Batch para garantir atomicidade
            const batch = db.batch();

            // Deletar documentos antigos
            snapshot.forEach(doc => {
                batch.delete(db.collection('playlist_tracks').doc(doc.id));
            });

            // Criar novos documentos paginados
            for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
                const docRef = db.collection('playlist_tracks').doc(); // gera ID
                batch.set(docRef, {
            ...
            // Atualizar contagem no documento da playlist
            const playlistRef = db.collection('custom_playlists').doc(this.currentPlaylistId);
    ...
    async deleteTrack(trackId) {
        ...
        const isConfirmed = await window.showCustomConfirm('Deletar Música', `Deletar "${track.title}"?`);
...
window.openAddMusicModalFromEdit = function() {
    console.log('🔥 openAddMusicModalFromEdit INICIADO');
    const manager = window.playlistEditManager;
    if (!manager) {
        console.error('❌ manager não encontrado');
        return;
    }
    if (!manager.currentPlaylistId) {
        console.error('❌ currentPlaylistId está vazio');
        return;
    }
    
    console.log('✅ Playlist ID:', manager.currentPlaylistId);
    
    // Auto-select the playlist in the dropdown
    const select = document.getElementById('selectPlaylistForMusic');
    if (select) {
        select.value = manager.currentPlaylistId;
        console.log('✅ Valor do select atualizado');
        try {
            select.dispatchEvent(new Event('change'));
            console.log('✅ Evento change disparado');
        } catch (e) {
            console.error('❌ Erro ao disparar change:', e);
        }
    } else {
        console.error('❌ selectPlaylistForMusic não encontrado');
    }
    
    // Forçar exibição do form (caso o evento falhe)
    ...
    if (addMusicModal) {
        // Remover do contexto atual e mover para o body para evitar problemas de stacking context (z-index)
        if (addMusicModal.parentElement !== document.body) {
            document.body.appendChild(addMusicModal);
            console.log('✅ Modal movido para o body (Stacking Context corrigido)');
        }
        
        addMusicModal.style.display = 'flex';
        // Forçar z-index absurdamente alto via JS
        addMusicModal.style.zIndex = '999999';
        console.log('✅ Modal exibido (display: flex)');
    } else {
        console.error('❌ addMusicModal não encontrado');
    }
};