    ...
    allTabs.forEach(tab => {
        ...
        tab.addEventListener('click', function handleTabClick() {
            ...
            if (targetContent) {
                ...
                if (targetTab === 'edit' && !targetContent.dataset.loaded) {
...
function initTimelineForms() {
    ...
    addTimelineForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            btn.innerHTML = '<i class="fas fa-cloud-upload-alt fa-spin"></i> Enviando para ImgBB...';
        ...
        <div class="details-content">
            ...
            <div class="details-info">
                <div class="detail-row editable">
                    ...
                    <input type="text" id="detailsTitle" class="detail-input" placeholder="Título da música">
                </div>
                <div class="detail-row editable">
                ...
                <div class="detail-row editable">
                    <strong>💿 Álbum:</strong>
                    <input type="text" id="detailsAlbum" class="detail-input" placeholder="Nome do álbum (opcional)">
                </div>
...
function addStarMapTabToAdmin() {
    ...
    if (document.querySelector('[data-tab="star-map"]')) return;
    
    const starMapTab = document.createElement('button');
    starMapTab.className = 'admin-tab';
    starMapTab.setAttribute('data-tab', 'star-map');
    starMapTab.innerHTML = '<i class="fas fa-star"></i> Mapa das Estrelas';
    tabsContainer.appendChild(starMapTab);
    
    const starMapContent = document.createElement('div');
    starMapContent.className = 'admin-content';
    starMapContent.id = 'star-map-tab';
    starMapContent.innerHTML = `
        <div class="star-map-config-card">
            <div class="card-icon-header">
                ...
                <h3>Configurações do Mapa das Estrelas</h3>
            </div>
            ...
            <form id="starMapConfigForm" class="modern-form">
                ...
                <div class="input-group">
                    ...
                    <textarea id="romanticQuote" rows="3" placeholder="Ex: O céu quando nossos mundos se colidiram"></textarea>
                    <small style="color: var(--theme-text-secondary); margin-top: 5px; display: block;">
                        Esta frase aparecerá no Mapa das Estrelas
                    </small>
                </div>
                ...
                <div class="constellation-preview" id="constellationPreview">
                    <div class="preview-header">
                        ...
                        <span>Preview das Constelações</span>
                    </div>
        ...
        <div class="star-map-info-card">
            ...
            <div class="info-content">
                <h4>Como funciona?</h4>
                <ul>
                    <li><strong>Data Especial:</strong> O sistema calculará as 3 constelações principais visíveis nesta data</li>
                    <li><strong>Localização:</strong> Influencia quais constelações aparecem (hemisfério norte/sul)</li>
                    <li><strong>Preview:</strong> Clique em "Calcular Preview" para ver quais constelações serão exibidas</li>
                </ul>
            </div>
        </div>
    `;
    
    contentArea.appendChild(starMapContent);
    
    setupTabListeners();
    
    // Event Listeners
    document.querySelectorAll('input[name="locationType"]').forEach(radio => {
        radio.addEventListener('change', toggleManualLocation);
    });
    
    document.getElementById('calculatePreviewBtn').addEventListener('click', calculateConstellationPreview);
    document.getElementById('starMapConfigForm').addEventListener('submit', saveStarMapConfig);
    
    // Carregar configurações existentes
    loadStarMapConfig();
    
    console.log('✅ Tab Mapa das Estrelas criada');
}
...
async function calculateConstellationPreview() {
    const btn = document.getElementById('calculatePreviewBtn');
    ...
    try {
        ...
        // Data
        if (specialDate) {
            date = new Date(specialDate + 'T12:00:00');
        } else {
            date = new Date();
        }
        ...
        if (locationType === 'manual') {
            ...
            if (isNaN(latitude) || isNaN(longitude)) {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-calculator"></i> Calcular Preview';
                return;
            }
        } else {
            // Usar geolocalização ou fallback
            try {
        ...
        // Calcular constelações (usar função do star-map.js)
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initStarMapAdminSystem);
} else {
