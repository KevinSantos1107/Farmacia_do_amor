...
function preloadInteractiveModules() {
    const preload = async () => {
        console.log('⏳ Preload de módulos interativos iniciado');

        if (!window.wordGameLoaded) {
            await loadScript('js/word-game.js');
            window.wordGameLoaded = true;
            console.log('✅ word-game.js pré-carregado');
        }

        if (!window.starMapLoaded) {
            ...
            console.log('✅ star-map pré-carregado');
        }

        if (!window.playlistEditLoaded) {
            await loadScript('js/playlist-edit.js');
            window.playlistEditLoaded = true;
            console.log('✅ playlist-edit.js pré-carregado');
        }

        console.log('✅ Preload de módulos interativos concluído');
    };
...
function initHamburgerMenu() {
    ...
    // Limpa qualquer estado persistente de admin antigo
    ...
    if (adminMenuBtn) {
        adminMenuBtn.addEventListener('click', async (e) => {
            ...
            if (window.adminState?.unlocked && typeof showAdminPanel === 'function') {
                showAdminPanel();
            } else if (typeof openAdminLogin === 'function') {
                openAdminLogin();
    ...
    const openStarMapBtn = document.getElementById('openStarMapBtn');
    if (openStarMapBtn) {
        openStarMapBtn.addEventListener('click', async function handleStarMapClick(e) {
            e.preventDefault();
    ...
    if (gameLink) {
        gameLink.addEventListener('click', async function handleWordGameClick(e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            closeMenu();
            ...
            if (window.wordGameReady) {
                await window.wordGameReady;
            }

            if (typeof window.WordGame?.openGame === 'function') {
                window.WordGame.openGame();
            }
        }, { once: true });
    }
