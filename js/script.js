...
class AlbumsCarousel3D {
    ...
    updatePositions() {
        ...
        if (window.LazyLoadManager) {
            window.LazyLoadManager.preloadCarouselCovers(this.currentIndex);
        }
    }
...
function openAlbum(albumId) {
    ...
    if (window.LazyLoadManager) {
        window.LazyLoadManager.preloadAdjacentPhotos(currentAlbum.id, currentPhotoIndex);
        
        const currentAlbumIndex = window.albums.findIndex(a => a.id === albumId);
        window.LazyLoadManager.preloadAdjacentAlbumsFirstLast(currentAlbumIndex);
    }
...
function updateAlbumViewer() {
    ...
    function changePhoto() {
        ...
        if (window.LazyLoadManager) {
            ...
            const isLastPhoto = currentPhotoIndex === currentAlbum.photos.length - 1;
            const isFirstPhoto = currentPhotoIndex === 0;
            
            if (isLastPhoto || isFirstPhoto) {
...
function goToNextAlbum() {
    if (!currentAlbum || !window.albums || window.albums.length <= 1) return;
    
    const currentIndex = window.albums.findIndex(a => a.id === currentAlbum.id);
    const nextIndex = (currentIndex + 1) % window.albums.length;
    const nextAlbum = window.albums[nextIndex];
    
    if (!nextAlbum || !nextAlbum.photos || nextAlbum.photos.length === 0) {
        console.warn('⚠️ Próximo álbum vazio');
        return;
    }
    
    console.log(`➡️ Indo para próximo álbum: ${nextAlbum.title}`);
    
    currentAlbum = nextAlbum;
    currentPhotoIndex = 0;
    
    if (albumsCarousel) {
        albumsCarousel.goToSlide(nextIndex);
    }
...
function goToPreviousAlbum() {
    ...
    const prevAlbum = window.albums[prevIndex];
    
    if (!prevAlbum || !prevAlbum.photos || prevAlbum.photos.length === 0) {
        console.warn('⚠️ Álbum anterior vazio');
        return;
    }
    
    console.log(`⬅️ Voltando para álbum anterior: ${prevAlbum.title}`);
    
    currentAlbum = prevAlbum;
    currentPhotoIndex = prevAlbum.photos.length - 1;
    
    if (albumsCarousel) {
        albumsCarousel.goToSlide(prevIndex);
    }
